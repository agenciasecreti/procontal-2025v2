...
import ModulesForm from '../../components/form';

// Função para atualizar o módulo
const updateModule = async (courseId: number, moduleId: number, formData: FormData) => {
...
export default function AdminModulesEditPage({
  params,
}: {
  ...
  const breadcrumbs = [
    ...
    { name: 'Editar Módulo', active: true },
  ];
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateModule(id, moduleId, formData)
      .then(({ success, error }) => {
        if (success) {
          toast.success('Módulo atualizado com sucesso!');
          setRefresh(new Date());
        } else {
          toast.error(error.message || 'Erro ao atualizar o módulo');
        }
      })
      .catch((error) => {
        toast.error(error.message || 'Erro ao atualizar o módulo');
        ...
        <div className="col-span-1">
          <div className="flex justify-between">
            <h1 className="mb-4 text-2xl font-bold">
              <small className="text-sm">Editar Módulo</small>
              <br />
              <Feature.icon className="mr-2 inline-block" /> {module?.title ?? Feature.name}
            </h1>
